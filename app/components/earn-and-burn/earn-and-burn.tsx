...
const EarnAndBurnRewards = () => {
    ...
    const handleScroll = (index: number) => {
        ...
        const itemWidth = firstItem?.getBoundingClientRect().width;
          scrollContainer.scrollTo({ left: index * (itemWidth || 300), behavior: 'smooth' });
        }
      };

      const handleRedeemReward = async (reward: Reward) => {
        const res = await redeemReward(customer?.id, reward.id, CAMPAIGNS.LOYALTY_PROGRAM_EARN_AND_BURN);
    ...
    return (
        <div className='py-4 overflow-x-auto' style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {rewardSuccessMessage && (
                <Toast
                    toastType="success"
                ...
                {rewards?.map(({ reward, assignment }, index) => (
                    ...
                    >   
                        ...
                        {redeemedReward.rewardId === reward.id && redeemedReward.barcodeUrl ? <Image
                                    ...
                                    width={250}
                                    height={65}
                                    className="max-w-auto max-h-auto mx-auto"/> : null}
                    </div>
                ))}
            </ScrollContainer>
            <div className="flex justify-center mt-6 gap-8">
                {rewards?.map((_, index) => (
                    ...
                    }} className="h-2.5 w-2.5 text-white border rounded-full" style={{ backgroundColor: activeIndex === index ? 'white' : 'inherit' }} />
                ))}
            </div>
        </div>
    )
